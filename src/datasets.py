...
class TrainDataset(Dataset):
    def __init__(self, image_root_dir, mask_root_dir=None, mode='train', size=256):
        super().__init__()
        self.mode = mode
        self.transform = transform_256()
        self.image_paths = []
        ...
        if mode == 'train':
            print(f"Size of train dataset: {len(self.image_paths)}")
            ...
            print(f"Size of val dataset: {len(self.image_paths)}")
    ...
    def __getitem__(self, index: int):
        ...
        if self.mode == 'train':
            mask = RandomMask(s=256)
            mask = torch.from_numpy(mask)
            
        elif self.mode == 'val':
            if hasattr(self, 'mask_paths') and len(self.mask_paths) > 0:
